...
#define ANA_SENS_R ad4 // ad4 befor "ad3"
#define SENS_RR !p9_2
#define GATE !p9_2
...
#define THRESHOLD_R_H 700 // 右アナログセンサ(0～1023)白判定しきい値 ～以上
#define THRESHOLD_R_L 200 // 右アナログセンサ(0～1023)黒判定しきい値　～以下

#define THRESHOLD_L_H 700 // 左アナログセンサ(0～1023)白判定しきい値  ～以上
#define THRESHOLD_L_L 200 // 左アナログセンサ(0～1023)黒判定しきい値  ～以下
...
/*共通(グローバル)変数*/
// 内輪差関係配列
int revolution_difference_fi[] = {
    100, 98, 97, 95, 93, 92, 90, 89, 87, 86,
    84, 83, 81, 80, 79, 77, 76, 75, 73, 72,
    71, 70, 69, 67, 66, 65, 64, 63, 62, 61,
    60, 59, 58, 57, 57, 56, 55, 54, 53, 52,
    52, 51, 50, 49, 49, 48, 47, 47, 46, 45,
    45, 44, 44, 43, 43, 42, 42, 41, 41, 40, 40};
int revolution_difference_ri[] = {
    100, 98, 97, 95, 93, 91, 90, 88, 86, 84,
    83, 81, 79, 78, 76, 74, 73, 71, 69, 67,
    66, 64, 62, 61, 59, 58, 56, 54, 53, 51,
    49, 48, 46, 45, 43, 42, 40, 38, 37, 35,
    34, 32, 31, 29, 28, 26, 25, 23, 22, 20,
    19, 18, 16, 15, 13, 12, 10, 9, 8, 6, 5}; // 54
int revolution_difference_ro[] = {
    100, 100, 100, 100, 100, 100, 99, 99, 99, 99,
    99, 98, 98, 98, 97, 97, 97, 96, 96, 95,
    95, 94, 94, 93, 93, 92, 92, 91, 91, 90,
    89, 89, 88, 87, 87, 86, 85, 85, 84, 83,
    82, 82, 81, 80, 79, 78, 78, 77, 76, 75,
    74, 73, 72, 72, 71, 70, 69, 68, 67, 66, 65};
 ...
 while (1) {
    ...
    { // 無限ループ
        ...
        if (pattern >= 11 && pattern <= 50)
        {
            ...
            // 左ハーフラインチェック
            if (check_leftline() && abs(getServoAngle()) < 15)
            {
                cnt1 = 0;
                laneMode = 1;
                pattern = 151;
            ...
            /*
                        // 右ハーフラインチェック
                        if (check_rightline() && abs(getServoAngle()) < 15)
                        {
                            cnt1 = 0;
                            laneMode = 1;
                            pattern = 151;
        ...
        case 11:
            ...
            else if (i > 15)
            {
                ...
                {
                    motor_f(diff_fi(95), 95);
                    motor_r(diff_ri(95), diff_ro(95));
                }
            }
            ...
            else if (i < -15)
            {
                ...
                {
                    motor_f(95, diff_fi(95));
                    motor_r(diff_ro(95), diff_ri(95));
                }
            }
        ...
        case 12: //	ストレート後のブレーキ処理　①　フロントブレーキ強め　100mm程度
            ...
            {                       // 右コーナー
                ...
                {                   //------強ブレーキ 4.7[m/s]
                    ...
                    cnt1 = 0;
                    motor_f(65, -65); // 前（左,右)
                    motor_r(45, -60); // 後（左,右)(-65,-65)
                }
                else if (58 <= iEncoder)          // 65
                {                                 //-中ブレーキ 4.3[m/s]
                    pattern = 31;                 // pattern = 31;
                    lEncoderBuff = lEncoderTotal; //    kyori = 225;        // 150mm
                    motor_f(40, -40);             // 前（左,右)   元 motor_f(-40, -40);
                    motor_r(30, -30);             // 後（左,右)   元 motor_r(-30, -30);
                }
                else
                {                                 //-------------弱ブレーキ
                    pattern = 41;                 // pattern = 41;
                    lEncoderBuff = lEncoderTotal; //    kyori = 151;        // 100mm
                    motor_f(15, -15);             // 前（左,右)  元　 motor_f(-15, -15);
                    motor_r(10, -10);             // 後（左,右)　元　　motor_r(-10, -10);
                }
            }
            break;
        ...
        case 21:                    //	ストレート後の強ブレーキ処理
            servoPwmOut(iServoPwm); // ライントレース制御
            i = getServoAngle();    // ステアリング角度取得
            if (lEncoderTotal - lEncoderBuff >= 325)
            { // 200m
                // if(cnt1>30){
                lEncoderBuff = lEncoderTotal;
                pattern = 23;
        ...
        case 23:                    //	ストレート後の強ブレーキ処理
            ...
            motor_f(-25, -25); // 前（左,右)(-65,-65)
            motor_r(-40, -40); // 後（左,右)(-35,-35)

            if (lEncoderTotal - lEncoderBuff >= 225)
            { // 150m
                lEncoderBuff = lEncoderTotal;
                pattern = 24;
                break;
        ...
        case 24:                    //	ストレート後の強ブレーキ処理
            ...
            if (lEncoderTotal - lEncoderBuff >= 250)
            { // 375mm
                mtPower = CORNER_START_POWER;
                pattern = 25;
                break;
            }
        ...
        case 25:                    //	ストレート後の強ブレーキ処理
            ...
            if (angleStreatCheck(i, STREAT_JUDGE_ANGLE) == 1)
            {                 // true
                pattern = 11; // 通常処理へ
            }
        ...
        case 33:                    //	ストレート後の中ブレーキ処理
            ...
            if (lEncoderTotal - lEncoderBuff >= 263)
            { // 150m
                pattern = 34;
                break;
            }
        ...
        case 34:                    //	ストレート後の中ブレーキ処理
            ...
            if (lEncoderTotal - lEncoderBuff >= 150)
            { // 200mm(300) →　100m(150)　（徐々に加速）
                mtPower = CORNER_START_POWER + 5;
                pattern = 35;
                break;
            }
        ...
        case 104: // クロスライン後の処理(1段目の減速処理)
            servoPwmOut(iServoPwm);
            if (iEncoder >= data_buff[DF_SP_CL] + 3)
            { /* エンコーダによりスピード制御 */
                motor_f(-25, -25);
                motor_r(-50, -50);
            }
            ...
            if (lEncoderTotal - lEncoderBuff >= 200)
            {
                // 200m
                pattern = 106;
                break;
        ...
        case 106: // クランク処理 (2段目の減速処理)　ハーフライン検出
            ...
            if (iEncoder >= data_buff[DF_SP_CL] + 2)
            { /* エンコーダによりスピード制御 */
                motor_f(-10, -10);
                motor_r(-10, -10);
            }
        ...
        case 110: // クランク処理	 　ハーフライン検出後
            ...
            {                              // クランク方向　左
                ...
                if (sensLLon == ON && ANA_SENS_L < THRESHOLD_L_L && (lEncoderTotal - lEncoderBuff) >= 75)
                {
            ...
            {                               // クランク方向　右
                ...
                if (sensRRon == ON && ANA_SENS_R < THRESHOLD_R_L &&
        ...
        case 112:
            ...
            { // クランク方向　左
                ...
                if (SENS_C == OFF && ANA_SENS_R < THRESHOLD_R_L &&
                    SENS_RR == OFF && ANA_SENS_L < THRESHOLD_L_L &&
                    cnt1 > 20)
                {
                    pattern = 114;
                }
            ...
            { // クランク方向　右
                ...
                if (SENS_C == OFF && ANA_SENS_R < THRESHOLD_R_L &&
                    SENS_LL == OFF && ANA_SENS_L < THRESHOLD_L_L &&
                    cnt1 > 20)
                {
                    pattern = 114;
                }
        ...
        case 116: // クランク処理
            ...
            {                              // クランク方向　左
                ...
                if (sensLLon == ON && ANA_SENS_L < THRESHOLD_L_L)
                {
                    pattern =
        ...
        case 152:                // クロスライン後の処理(白線トレース時)
            ...
            if (iEncoder >= data_buff[DF_SP_RC] + 3)
            { /* エンコーダによりスピード制御 */
                motor_f(-20, -20);
                motor_r(-30, -30);
            }
            ...
            if (SENS_RR == OFF && SENS_LL == OFF && SENS_C == OFF && ANA_SENS_L < THRESHOLD_L_L && ANA_SENS_R < THRESHOLD_R_L)
            {
                pattern = 154; // 全　てのセンサ　黒検出時次の処理へ
            }
...
void init(void)
{
    ...
    p1 = 0x00;
    pd1 = 0x90; // 1101 0000  1001 0000 p1 7654 3210
...
void intTRC(void)
{
    ...
    if (pattern >= 11 && pattern <= 230 && lEncoderTotal >= 1515L * 3)
    {
        /* 距離による停止処理 */
        // モータSW(ON)：0なら走行
 ...
 if (ANA_SENS_L > 300 && ANA_SENS_L > 300) {
    ...
    case 1:
        /* エンコーダ制御 */
        i = trg;                     // trg 65535 → 0 → 1
        iEncoder = i - uEncoderBuff; // 20-65535
        if (iEncoder < 0)
        {
            iEncoder = 65535 + iEncoder;
        }
        lEncoderTotal += iEncoder;
        // lEncoderTotal=lEncoderTotal+iEncoder
        uEncoderBuff = i;
        break;
...
int check_crossline(void)
{
    ...
    //        ret = 1;
    //    }
    if (sensLLon == ON && SENS_C == ON && sensRRon == ON)
    {
...
int getAnalogSensor(void)
{
    ...
    if (((sensLLon == ON && SENS_C == ON) || (sensRRon == ON && SENS_C == ON)) && pattern != 3)
    {
        //        ret = 0;
    }
...
int courceOut(void)
{
    ...
    else if (ANA_SENS_L > THRESHOLD_L_H)
    { /* Lセンサ白線 */
        switch (cource)
        {
        case 0:
    ...
    else if (ANA_SENS_R > THRESHOLD_R_H)
    { /* Rセンサ白線 */
        switch (cource)
        {
        case 0:
    ...
    { /* RRセンサ白線 */
        ...
        case 3:
            ...
            break;
             ...
             ANA_SENS_R < THRESHOLD_R_L &&
             ANA_SENS_L < THRESHOLD_L_L)
    { /* 全てのセンサ黒 */
        switch (cource)
        {
        case 2:
