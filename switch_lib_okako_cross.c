...
/************************************************************************/
static void sw_nop( volatile int i )
{
    while( i-- );
}
/************************************************************************/
...
現在の設定 bit  7    6    5    4    3    2    1    0
              P0_7  ---  ---  --- P0_3 P0_2 P0_1 P0_0
*/
    volatile unsigned short int sw;

    prc2 = 1;                           /* PD0のプロテクト解除          */
    pd0 &= 0x70;                        /* LCDと兼用の端子を入力に設定  */
	sw_nop(10);
    sw = (~p0 & 0x0f);                  /* P0_7 P0_3～P0_0読み込み      */

    prc2 = 1;                           /* PD0のプロテクト解除          */
    pd0 |= 0x0f;                        /* LCDと兼用の端子を出力に設定  */
    return sw;
}
