    ...
    while (1)
    {
        ...
        case 110: // クランク処理	 　ハーフライン検出後
            ...
            {                              // クランク方向　左
                ...
                if (SENS_LL == ON && ANA_SENS_L < THRESHOLD_L && (lEncoderTotal - lEncoderBuff) >= 75)
                {
            ...
            {                               // クランク方向　右
                ...
                if (SENS_RR == ON && ANA_SENS_R < THRESHOLD_L &&
        ...
        case 116: // クランク処理
            ...
            {                              // クランク方向　左
                ...
                if (SENS_LL == ON && ANA_SENS_L < THRESHOLD_L)
                {
                    pattern =
...
#if 1
        if (SENS_LL == ON && SENS_C == ON && SENS_RR == ON)
        {
            check_sen_cnt++;
...
int getAnalogSensor(void)
{
    ...
    if (((SENS_LL == ON && SENS_C == ON) || (SENS_RR == ON && SENS_C == ON)) && pattern != 3)
    {
        ret = 0;
    }
...
int courceOut(void)
{
    ...
    else if (ANA_SENS_L > THRESHOLD_H)
    { /* Lセンサ白線 */
        switch (cource)
        {
        case 0:
    ...
    else if (ANA_SENS_R > THRESHOLD_H)
    { /* Rセンサ白線 */
        switch (cource)
        {
        case 0:
...
int lcdProcess(void)
{
    ...
    case 2:
        ...
        lcdPosition(0, 1);
        lcdPrintf("L=%1d Center=%1d R=%1d %1d   ", SENS_LL, SENS_C, SENS_RR,
                  SENS_ALL);
        cnt1 = 0;
        break;
